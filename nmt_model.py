...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        dec_X = torch.cat((last_hidden[0], last_hidden[1]), dim = 1)
        init_decoder_hidden = self.h_projection(dec_X)
        dec_c = torch.cat((last_cell[0], last_cell[1]), dim = 1)
        init_decoder_cell = self.c_projection(dec_c)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #enc_hiddens_proj = self.att_projection(enc_hiddens_proj)
