...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_to_char_idxs = self.words2charindices(sents)
        pad_sent_to_char_idxs = pad_sents_char(sent_to_char_idxs, self.char_pad) #(batch_size, max_sentence_length, max_word_length)
        pad_sent_to_char_idxs = torch.tensor(pad_sent_to_char_idxs, dtype=torch.long, device=device)
        return pad_sent_to_char_idxs.permute(1, 0, 2).contiguous() #.contiguous().view(char_vars.size()[1], char_vars.size()[0], char_vars.size()[2])
